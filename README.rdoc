     ...
     * Tym razem zajme sie edycja naszego koszyka
       ...
       ** Idzimy teraz do naszego formularza  cart/edit  w ktorym umozliwimy takie rzeczy jak
           ...
           Okazuje sie ze RAILSY dodaly mozliwosc edycji wielu obiektów naraz w jednym formularzu
            ...
            - Teraz kontroller i ustawmy nasza zmienna
            ...
            - Przejdzmy teraz do widoku edit controllera cart
              viws/cart/edit.html.erb
              - i tutaj umiescimy nasz formularz do adresu, wyboru dostawy, itd...


                   I tutaj pojawia sie bardzo ciekawa rzecz:
                     - czyli na zmiennej f wywolujemy metode simple_fields_for - jest to metoda od simple forma i ona
                       powoduje wlasnie obsluzenie tych zagniezdzonych atrybutow  (accepts_nested_attributes_for :address )
                       W momencie kiedy bysmy uzywali zwyklego formularza czyli form_form to w tym przypadku bysmy uzyli        ->  fields_for
                         czyli nie musimy uzywas simple_forma aby obsluzyc zagniezdzone atrybuty
                         zwykle railsowe formularze również to obsluguja
                         ...
                         <div class="row">
                           <div class="col-md-6">
                             <%= simple_form_for @cart, url: cart_path do |f| %>
                                 <%= f.simple_fields_for :address do |a| %>           <!-- I tutaj pojawia sie bardzo ciekawa rzecz, wywoluje metode simple_fields_for i podajej nazwe assocjaci i tworzymy nowy obiekt formularza i to bedzie taki podformularz  -->
                                     <%= a.input :email %>                            <!-- I na tym podformularzy wywolujemy kolejne metody aby zbudowac inputy dla konkretnych pol tego podformularza   -->
                                     <%= a.input :first_name %>                       <!-- Wazne jest to zeby te inputy wywolywac na podformularzu  -->
                                     <%= a.input :last_name %>
                                     <%= a.input :city %>
                                     <%= a.input :zip_code %>
                                     <%= a.input :street %>
                                 <% end %>
                                 <%= f.association :shipping_type, as: :select, prompt: nil %>   <!-- W tym miejscu wybieramy rodzaj dostawy -->
                                 <%= f.input :comment %>                                         <!-- Dodajemy mozliwosc skomentowania zamowienia -->
                                 <%= button_tag class: "btn btn-success btn-lg" do %>            <!-- Dodajemy przycisk ktory zapise te dane i pozwoli przejsc dalej-->
                                     Zapisz i przejdź dalej                                      <!-- Czyli bedzie prowadzil do akcji update i w momencie kiedy akcja zostanie wywolana to wysztkie dane zostana zaktualizowane i uzytkownik bedzie docelowo przekierowany do akcji confirmation gdzie bedzie mógł potierdzic zamowienia  -->
                                     <span class="fa fa-step-forward"></span>
                                 <% end %>
                             <% end %>
                           </div>
                   ...
                   I gdy wejdziemy na formularz dostawy   http://localhost:3000/cart/edit
                   to widzimy ze wyglada w oczekiwany sposób
                    - poniewaz mamy walidacje na niektore pola to simple_form dodaje nam gwiazdke


            - Dodajmy teraz akcje update
              w cart_controller.rb
            ...
            - Teraz wyslijmy putsty formularz z nieprawidlowymi danymi
                      http://localhost:3000/cart

                      i widzimy ze walidacaj dziala poprawinie
                      - wypelnijmy formularz
                      - i klikamy do przejdz dalej
                      - i udalo nam sie dojsc do kroku konfirmation



       - Dotarlismy do potwiedzenia zamowienia - Confirmation
         * Routes
           - dodajmy przycisk prowadzacy do potwierdzenia zamowienie i nazwijmy do finisz
                        resource :cart, controller: "cart", only: [:show, :update, :edit] do    # resource pozwala nam okreslic doklanie do jakiego kontrollera ma prowadzic i okreslic jakie sciezki ma zawierac
                           member do                                                             # i na konkretnym koszyku chcemy miec takie metody
                             post :add_product                                                   # metoda sluzaca do dodawania produktu
                             post :remove_product                                                # metoda do usówania produktu
           ...
           - zainicjumy teraz nasz widok views/cart/confirmation.html.erb
                     ...
                     <table class="table table-striped">
                       ...
                       <% end %>
                       <tr>
                         <td>Dostawa: <%= @cart.shipping_type.name %></td>                  <!-- Dodatkowo dodajemy koszt dostawy -->
                         <td></td>
                         <td colspan="2"><%= number_to_currency @cart.shipping_cost %></td>  <!-- Wyswietlamy cene i rodzaj dostawy -->
                       </tr>
                       <tr>
                         <td colspan="4"><hr/></td>
                       </tr>
                       <tr>
                         <th>Całość</th>
                         <td></td>
                         <td colspan="2"><%= number_to_currency @cart.full_cost %></td>       <!-- Calkowity koszt -->
                       </tr>
                       </tbody>
                     </table>

                     <%= link_to finish_cart_path, method: :post, class: "btn btn-success btn-lg" do %>   <!-- link prowadzacy do akcji ktora sklada to zamowienie -->
                         <span class="fa fa-check"></span>
                         Potwierdzam zamówienie z obowiązkiem zapłaty
                     <% end %>


           - gdy odswiezymy widzimy ze zmienna cart jest nilem - spr, czemu tak sie stalo
           - poniewaz akcja confirmation jest zupelnie PUSTA  w kontrolerze

                        def confirmation
                            @cart = current_cart      # standardowo musimy ustawic na aktualny koszyk
                        end

           - gdy odswiezymy, mamy oczekiwany efekt




       * Teraz ostatni krok czyli zaimplementowanie akcji finish
        - i ta akcja bedzie robila
        ...
        - Zerknijmy jeszcze na maszyne stanow zwiazana z zamowieniem
                 ...
                 models/order_state_machine.rb
                         ...
                         widzimy gdy w momencie stan zostanie zmieniony na confirm
                         to zostanie wyslany email do uzytownika


        - Szablony do naszych zamowien przechowywane sa w :
                          views/order_mailer/order_confirmation.html.erb

                          - zmienmy go aby wyswietlil nam sie jakis sensowny komunikat

                                  <p>Dziękujemy za zamówienie!</p>

        - Uzupelnmy teraz pozostale szablony do maili, cancelled, in_progres, shipped



     - Sprawdzimy czy dziala klikajac przycisk potwierdzajacy zamowienie
     - nie
             - mamy blad
             - zmienmy w models/order_transitons.rb

                 class OrderTransition < ActiveRecord::Base
                   include Statesman::Adapters::ActiveRecordTransition


                   belongs_to :order, inverse_of: :transitions                             # Tu na transitions
                   #i zmienimy tu jedna rzecz:
                   # Widzimy ze jest napisane order_transitions ale my zmienimy na transitions
                   #       zmieniamy z :   belongs_to :order, inverse_of: :order_transitions
                   #                 na:   belongs_to :order, inverse_of: :transitions
      ...
      - i widzimy ze letter_opener wyslal nam maila - czyli dokladnie tak jak zadeklarowalismy w naszej maszynie stanow
        a na stronie głowniej mamy komunikat dziekujemy za zamowienie




9.14 Panel Admina - zarzadzanie zamowieniami
     * Brakuje nam sekcji w panelu admina aby zarzadzac zamowieniami
     * Zatem wygenerujmy nasz kontroller
       - nasz kontroller bedzie zawarty w przestrzenie nazw admin
       - i biedzie zawierał nastepujace akcje

           rails g controller admin/orders index show

     * Teraz do naszego pliku z sciezkami musimy dodac odpowiednie sciecki - Routes
     ...
     * Dodatkowo musimy pamietac aby otworzyc layouts admin i dodac link prowadzacy do zamowien
        ...
        - Sprawdzmy czy smiga   http://localhost:3000/admin
          : widzimy ze pojawila sie sekcja zamowienia


     * Idzmy teraz do kontrollera  admin/orders_controller.rb
       - akcja index bedzie bardzo prosta

       % wywolamy metode od Statesmana ktory zwroci nam wszytkie zamowienia ktore juz sa zlozone
         a nie sa koszykami
         i uzyjemy metody od Statesmana ktora sie nazywa not_in_state i wymieniamy w ktorych stanach me tej metody nie byc
         nie chcemy jej wylacznie w stanie new

               def index
                  @orders = Order.not_in_state(:new).page(params[:page]).per(20)
               end

     * Skoro mamy juz routsy , controller, to przejdzmy do widoku akcji index naszego zamówienia
        - mamy tu nic specjalnego
        - zwyczajajna tabelka wymieniajca wszytkie zamowienia
        - mamy paginacjie
        - linka do szczegolów zamówienia
        - i szczegóły zamowienia tj: pelny koszt,aktualny stan, sformatowana data zamowienia i nr.id
               ...
               <table class="table table-striped">
                 ...
                 <% @orders.each do |order| %>
                     <tr>
                       ...
                       <td>
                         <%= link_to "Edycja", admin_order_path(order), class: "btn btn-default btn-xs" %>
                       </td>
                     </tr>
                 <% end %>
                 </tbody>
     ...
     * Jak odsiwezymy strone widzimy ze wszystko smiga http://localhost:3000/admin/orders
       - wyswitelmy statsu zamowienia po polsku  - confirmed

       - nasz state_machine ma metode states_map
         ktora mampuje rubiowe nazwy na polskie odpowiedniki
         ...
         a zeby nam sie wyswietlalo po polsku musimy zrobic takie cos:)

            <td><%= OrderStateMachine.states_map[order.current_state] %></td>   <!-- Zmieniamy na polska nazwe. Czyli Pobieramy wartosc pod kluczem ktorego nazwa jest aktualny stan   -->


         - czyli nasz widok bedzie wygladal tak:
         ...
         - i gdy odswiezymy widzimy ze status jest przetlumaczony na polski i ze jest zlozony przez klijenta



     ** Teraz wyswitlmy szczegóły danego zamowienia oraz formularza do zmiany statusu zamowienia tak aby
        administrator mógł np, stwierdzic ze zamowienie jest w realizacji lub jest wyslane

        - akcja show w kontrollerze admin/orders_controllers/rb

          - jest ona dosc trywialna, po prostu znajdujemy zamówienie po id
        ...
        - Przejdzmy teraz do widoku wievs/admin/orders/show
          Po lewej stronie beda pokazane szczegoly koszyka i pokazemy dokladnie te same informacjie
          ktore widzial uzytkownik w momencie potwierdzenia zamowienia.
          Podajmey rodzaj i cene dostawy.
          Natomiast po prawej stronie bedziemy wyswietlac adres uzytkownia oraz komentarz
        ...
        - gdy odswiezymy strone widzimy oczekiwany efekt http://localhost:3000/admin/orders/3

     * Obsluzymy teraz stan z nszej maszyny
       - dodajmy metode update w naszym kontrolerze controllers/admin/orders_controller.rb
       ...
       - i gdy teraz uruchomimy api i zmieniemy na stan przyjety do realizacji
         widzimy ze zostal wyslany email do klijeta jak również został wyswietlony komunikat



  ****** I TUTUAJ POPELNILISMY KRYTYCZNY BLAD - KTORY NIE MOZE MIEC MIEJSCA  czyli:

         Nasz konroller uzywa layoutsu głownego sklepu
         - a wiec nie dziedziczy z bazowej klasy dla admina
         - krotko mówiac ta sekacja jest dostepna dla zwyklych uzytkownikow

         Za kazdym razem gdy implementujemy taka sekcjie musimy sie upewnic ze jest ona
         odpowiednio zabezpieczona:
         - przejdzmy do naszego kontrollera
         - widzimy ze nie jest zabezpieczony
         ...
         - gdy teraz odswiezymy strone widzimy ze nasz widok pochodziu juz z panelu admina
         - i dzieki tej operacji widzimy,ze nasz kontroller jest juz zabezpieczony haslem
           poniewaz wszystkie kontrollery dziedziczace po BaseController sa zabezpieczone haslem



         - Widzimy ze nasze zamowienie jest przyjete do realizacji
           spr. wyslac to zamowienie
           otrzymujemy maila ze twoje zamoienie zostalo wyslane
           i gdy teraz wejdziemy do wszystkich naszych zamowien to widzimy ze zamowienie zostalo wyslane
         - A gdybysmy chcieli wyslac jeszcze raz to zamowienie
           to gdy wejdziemy do edycji to widzimy ze maszyna stanow wyłaczyła juz niektóre stany
           i nie pozwala przejsc do tych stanow
           http://localhost:3000/admin/orders/3
         - sprobujemy jeszcze anulowac - i tu tez dziala dobrze

9.16 Pomysly na rozwój sklepu
